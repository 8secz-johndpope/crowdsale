...
contract VestingWallet is Ownable {
        ...
        pastCliffTime(msg.sender)
    {
        ...
        uint amountWithdrawable = SafeMath.sub(totalAmountVested, vestingSchedule.totalAmountWithdrawn);
        ...
        addressNotNull(_addressToRefund)
    {
        ...
        if (block.timestamp < vestingSchedule.cliffTimeInSec) {
            ...
            uint totalAmountVested = getTotalAmountVested(vestingSchedule);
            amountWithdrawable = SafeMath.sub(totalAmountVested, vestingSchedule.totalAmountWithdrawn);
            amountRefundable = SafeMath.sub(vestingSchedule.totalAmount, totalAmountVested);
        }
        ...
        returns (uint)
    {
        ...
        uint timeSinceStartInSec = SafeMath.sub(block.timestamp, vestingSchedule.startTimeInSec);
        uint totalVestingTimeInSec = vestingSchedule.endTimeInSec.sub(vestingSchedule.startTimeInSec);
        uint totalAmountVested = 
            (timeSinceStartInSec.mul(vestingSchedule.totalAmount)).div(
